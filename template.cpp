...
template <> inline void fw(string ch) {
    for (auto c : ch) fw(c);
}
template <typename T> inline void fwn(T num) { fw(num), fw(_end); }
template <typename T, typename... Args> inline void fwn(T num, Args... args) { fw(num), fw(_sep), fwn(args...); }
//	struct Printer{
//		char _end,_sep;
...
_Atomic_word main(int argc, char* argv[]) {
    if (argc > 1) freopen("in.txt", "r", stdin), freopen("out.txt", "w", stdout);
